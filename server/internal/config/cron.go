...
// cronParser mirrors the field spec poolscheduler.NewPoolScheduler
// configures via cron.WithSeconds() -- kept in sync deliberately, so that
// a schedule accepted here (at save time) can never be rejected by the
// scheduler itself at run time, or vice versa.
...
// e.g. "59 59 23 * * FRI"). Used to reject a bad poolSchedule entry at
...
// detect a duplicate poolSchedule entry for the same miner.
